...
def convert_to_tsv(train_name, dev_name, test_name):
    ...
    train, dev, test, data = [], [], [], []
    train_out, dev_out, test_out = open('./data/'+os.path.basename(train_name).split('.')[-1]+'.tsv', 'w'), \
                                   open('./data/' + os.path.basename(dev_name).split('.')[-1] + '.tsv', 'w'),\
                                   open('./data/' + os.path.basename(test_name).split('.')[-1] + '.tsv', 'w') # SST2_TSV/
    files = []
...
if __name__ == '__main__':
    train, dev, test = convert_to_tsv('data/train.tsv', 'data/dev.tsv', 'data/test.tsv') #SST2_TSV/
    print(train)
